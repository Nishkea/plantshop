...
export default function PlantCard({title, thumbnail, price, amount, slug})
{
   return (
      <Link href={`/plant/${slug}`}>
         <article className="group col-span-3 flex flex-col justify-start items-start bg-white/80 p-2 rounded hover:cursor-pointer hover:shadow-xl transition-shadow duration-300">
            ...
            <p className={`text-sm font-medium py-1 px-2 rounded font-primary ${amount > 0 ? ' bg-green-600 text-white' : ' bg-red-800 text-red-300'}`}>{amount > 0 ? 'Op voorraad' : 'Niet op voorraad'}</p>
         </article>
      </Link>
   )
}